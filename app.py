...
# --- FUNÇÕES AUXILIARES DO BACKEND (CORRIGIDAS) ---
def hash_senha(senha):
    """Encripta a senha."""
    return hashlib.sha256(senha.encode('utf-8')).hexdigest()

def get_db_url(endpoint):
    """Gera a URL completa (CORRIGIDO para evitar barras duplas //)"""
...
# --- ROTA 3: API PARA REGISTO (COM VALIDAÇÃO CORRIGIDA) ---
...
def api_register():
    ...
    # Validação de E-mail (Formato CORRIGIDO)
    ...
    try:
        ...
        app.logger.debug(f"Tentativa de cadastro: {novo_usuario['email']}")
        response = requests.post(get_db_url('contas'), json=novo_usuario)
        app.logger.debug(f"Firebase: {response.status_code} - {response.text}")
...
def api_update_agendamento():
    ...
    if status == 'concluido':
        ...
        atendimento = {
            'id_funcionario': ag.get('id_funcionario'), 'id_cliente': ag.get('id_cliente'),
            'nome_cliente': ag.get('nome_cliente'), 'nome_servico': ag.get('nome_servico'),
            'valor_servico': ag.get('valor_servico'), 'comissao_percentual': comissao / 100,
...
def api_relatorio_geral():
    ...
    return jsonify({"faturamento_total": total, "total_comissoes": comissoes, "lucro_bruto": total - comissoes})
...
def api_analise_cliente(id_cliente):
    ...
    return jsonify({"total_visitas": visitas, "frequencia_msg": frequencia_msg, "cliente_desde": data_registo})
